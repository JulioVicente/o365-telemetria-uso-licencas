...
$results = foreach ($user in $users) {
    ...
    $saving = if ($unpriced.Count -eq 0) { [math]::Max([decimal]0, ([decimal]$currentPrice - [decimal]$recommendedPrice)) } else { $null }
...
$planActions = @(
    ...
    [pscustomobject]@{ID=6;Pilar='Renovacao';Acao='Revisar impacto dos reajustes e novos pacotes de 2026';Origem='Novidades 2026';Prioridade='Alta';Responsavel='BestSoft';Prazo='';Status='Em andamento';Evidencia="Oportunidade teorica mensal de R$ $($summary.estimatedMonthlySavingsBRL.ToString('N2')); catalogo $($catalogData.asOf)";ProximoPasso='Confirmar preco contratado, quantidade faturada, vigencia e possibilidade de reducao na renovacao';Fonte='Microsoft Brasil e Partner Center Announcements'},
...
<div class="cards"><div class="card"><div class="value">$(@($results).Count)</div><div class="label">Usuarios analisados</div></div><div class="card"><div class="value">$($summary.sharedMailboxCandidates)</div><div class="label">Candidatos a caixa compartilhada</div></div><div class="card"><div class="value">$($summary.removalCandidates)</div><div class="label">Candidatos a remocao</div></div><div class="card saving"><div class="value">R$ $($summary.estimatedMonthlySavingsBRL.ToString('N2'))*</div><div class="label">Oportunidade mensal teorica$(if($summary.usersWithUnpricedLicenses -gt 0){" (parcial; $($summary.usersWithUnpricedLicenses) usuarios pendentes de preco)"})</div></div></div>
<div class="note"><b>* Valor nao garantido e nao necessariamente imediato.</b> Calculado com precos publicos sobre licencas atribuidas e recomendacoes automaticas. Nao representa reducao confirmada da fatura. Depende da validacao de cada usuario, preco contratado, impostos, quantidade comprada e faturada, vigencia, modalidade mensal/anual/anual paga mensalmente e permissao de reduzir quantidades na renovacao. Licencas apenas desatribuidas podem continuar sendo cobradas enquanto permanecerem contratadas.</div>
